...
Article.fetchAll = () => {
  ...
  if (localStorage.rawData) {
    ...
    $.getJSON('data/hackerIpsum.json')
      .then(objects => {
        localStorage.setItem('rawData', JSON.stringify(objects))
        return objects
      })
      .then(objects => {
        Article.loadAll(objects)
        articleView.initIndexPage();
        //COMMENT Each code block of each if/else statement needed to be completed with the articleView.initIndexPage to be able to render. Both still need to render to the page, which is why it is in each.
      })
  }
}
